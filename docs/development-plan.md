...
| コマンド ID                     | 説明                                                         |
| ------------------------------- | ------------------------------------------------------------ |
| `branchPainter.setColor`        | 手動で色を選択する（カラーピッカーまたはプリセットから選択） |
| `branchPainter.resetColor`      | 色をリセットしてデフォルトに戻す                             |
| `branchPainter.toggleAutoColor` | 自動色分けの有効/無効を切り替え                              |

### 設定 (`contributes.configuration`)

| 設定キー                          | 型        | デフォルト値                                                            | 説明                                                                                |
| --------------------------------- | --------- | ----------------------------------------------------------------------- | ----------------------------------------------------------------------------------- |
| `branchPainter.enabled`           | `boolean` | `true`                                                                  | 拡張機能の有効/無効                                                                 |
| `branchPainter.affectTitleBar`    | `boolean` | `true`                                                                  | タイトルバーの色を変更するか                                                        |
| `branchPainter.affectStatusBar`   | `boolean` | `true`                                                                  | ステータスバーの色を変更するか                                                      |
| `branchPainter.affectActivityBar` | `boolean` | `true`                                                                  | アクティビティバーの色を変更するか                                                  |
| `branchPainter.showBranchInTitle` | `boolean` | `true`                                                                  | タイトルバーにブランチ名を表示するか                                                |
| `branchPainter.titleFormat`       | `string`  | `"[${branch}] ${folderName} ${separator} ${activeEditorShort}${dirty}"` | タイトル表示のフォーマット                                                          |
| `branchPainter.branchColorMap`    | `object`  | `{}`                                                                    | ブランチ名と色の手動マッピング（例: `{ "main": "#1e8a3a", "develop": "#3a7ae8" }`） |
| `branchPainter.saturation`        | `number`  | `0.6`                                                                   | 自動生成色の彩度                                                                    |
| `branchPainter.lightness`         | `number`  | `0.3`                                                                   | 自動生成色の明度                                                                    |
